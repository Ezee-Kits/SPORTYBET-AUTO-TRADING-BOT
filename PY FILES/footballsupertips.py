...
from func import drop_duplicate,save_daily_csv,match_day_date,saving_to_firebase,save_daily_csv2
...
def footballsupertips_func():
    ...
    path = path = f'{full_path}/footballsupertips.csv'
    ...
    all_df['1X'] = (((all_df['HOME PER'].astype(float)/100) + (all_df['DRAW PER'].astype(float)/100))*100).round()
    all_df['12'] = (((all_df['HOME PER'].astype(float)/100) + (all_df['AWAY PER'].astype(float)/100))*100).round()
    all_df['X2'] = (((all_df['AWAY PER'].astype(float)/100) + (all_df['DRAW PER'].astype(float)/100))*100).round()
    all_df['OVER 1.5'] = all_df['BTS'].astype(int) + 15
    
    print(all_df)
    all_df.to_csv(path, index=False)
    print('============================= FOOTBALLSUPERTIPS FILE SAVED ==========================')
    drop_duplicate(path=path)
    # saving_to_firebase(data=all_df,file_name='Footballsupertips')
